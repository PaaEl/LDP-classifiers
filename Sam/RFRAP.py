...
for x in database_names:
    ...
    epsilon = 10
    d = 10
    client_olh = RAPPORClient(epsilon=epsilon, d=d, use_olh=True)
    server_olh = RAPPORServer(epsilon=epsilon, d=d, use_olh=True)
    server_olh2 = RAPPORServer(epsilon=epsilon, d=d, use_olh=True)
    olh_estimates2 = np.array([0])
