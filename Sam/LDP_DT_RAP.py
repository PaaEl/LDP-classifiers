...
epsilon_values=[0.1,0.5,1,2,3,5,10]
depth = 4
...
def perturb(df, e):
    ...
    for x in df.columns:
        epsilon = e
        d = max(df[x]) + 1
        server_olh.update_params(f, 128, 8, d)
        client_olh.update_params(f, 128, 8, d, server_olh.get_hash_funcs())
...
for xx in database_names:
    ...
    for epsilon_value in epsilon_values:
        f = round(1 / (0.5 * math.exp(epsilon_value / 2) + 0.5), 2)
        f = 0.98
        server_olh = RAPPORServer(f, 128, 8, d)
        client_olh = RAPPORClient(f, 128, 8, d, server_olh.get_hash_funcs())
        T = X
        j = encode(X, c)
