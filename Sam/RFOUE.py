...
from pure_ldp.frequency_oracles import LHClient, LHServer, DEClient, DEServer, UEClient, UEServer
import DataPreprocessor
database_names=['adult','spect','car','nursery']
...
def recategorize(io, e, d):
    winner = np.argwhere(io == np.amax(io))
...
for x in database_names:
    ...
    epsilon = 10
    d = 10
    client_olh = UEClient(epsilon=epsilon, d=d, use_oue=True)
    server_olh = UEServer(epsilon=epsilon, d=d, use_oue=True)
    server_olh2 = UEServer(epsilon=epsilon, d=d, use_oue=True)
