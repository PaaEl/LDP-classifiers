...
def perturb(df, e):
    ...
    for x in df.columns:
        ...
        client_olh.update_params(epsilon, d, index_mapper=None, hash_funcs=server_olh.get_hash_funcs())
...
for x in database_names:
    ...
    epsilon = 10
    d = 10
    server_olh = HadamardResponseServer(epsilon, d)
    client_olh = HadamardResponseClient(epsilon, d, server_olh.get_hash_funcs())
    server_olh2 = HadamardResponseServer(epsilon=epsilon, d=d)
    olh_estimates2 = np.array([0])
